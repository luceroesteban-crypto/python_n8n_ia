...
--------------------------------------------------------------------------
CAMBIO respecto a la versión de "Clase 25" (documentado en `mejoras.md` del
proyecto original) — modelo de embeddings multilingüe
--------------------------------------------------------------------------
La versión anterior usaba `sentence-transformers/all-MiniLM-L6-v2`, que está
entrenado principalmente en inglés. Los documentos de este proyecto
(`datos.txt` y `documento.pdf`, con contenido en español) no se beneficiaban
de ese modelo: en las pruebas, preguntas relacionadas con el contenido de
`datos.txt` devolvían fragmentos irrelevantes de `documento.pdf` (y viceversa)
simplemente por compartir vocabulario general en español mal representado
por un modelo en inglés.

Por eso se cambia a:
    EMBEDDING_MODEL = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"

Se eligió este modelo (y no `BAAI/bge-m3` o `intfloat/multilingual-e5-large`)
porque:
- Es un reemplazo directo en tamaño/velocidad del modelo anterior (mismo
  orden de magnitud de parámetros), por lo que no cambia significativamente
  el tiempo de ingesta ni de consulta en CPU.
- Tiene soporte multilingüe real, incluyendo español, a diferencia del
  modelo anterior.
- No requiere prefijos especiales en el texto (como `"query: "` /
  `"passage: "` en los modelos E5), lo que simplifica el código de
  `consulta.py` / `consulta_filtrada.py`.

IMPORTANTE: al cambiar el modelo de embeddings es obligatorio volver a correr
esta ingesta para regenerar `db_chroma`. Los vectores generados con un modelo
distinto no son compatibles entre sí (dimensiones y espacio semántico
distintos), y mezclar embeddings de dos modelos en la misma colección
produciría resultados de similitud sin sentido.
"""
...
# Modelo multilingüe (ver justificación en el docstring del módulo).
EMBEDDING_MODEL = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
...
def main():
    ...
    # Usamos un modelo open-source de HuggingFace, multilingüe (ver
