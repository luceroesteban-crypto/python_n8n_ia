# =============================================================================
# database.py - DEMO EDUCATIVA: Capa de Acceso a Datos (DAO Pattern)
# =============================================================================
#
# OBJETIVO PEDAGÓGICO:
#   Este archivo demuestra el patrón DAO (Data Access Object) que separa
#   la lógica de acceso a datos de la interfaz de usuario.
#
# CONCEPTOS CLAVE PARA ESTUDIANTES:
#   - Conexión persistente vs conexiones por operación
#   - SQL dinámico con parámetros seguros (prevención de inyección SQL)
#   - CRUD: Create, Read, Update, Delete
#   - Placeholders %s (MySQL) vs ? (SQLite)
#   - Manejo de errores con messagebox en lugar de excepciones
#
# SEGURIDAD:
#   - NUNCA concatenar strings directamente en queries SQL
#   - Usar placeholders %s con parámetros separados en cursor.execute()
#   - Validar valores permitidos antes de usar en f-strings SQL
#
# =============================================================================
...
class DatabaseManager:
    ...
    def fetch_all_clientes(self, ordenar_por='nombre', direccion='ASC'):
        """
        Obtiene todos los clientes de la tabla ordenados según los parámetros.
        
        SEGURIDAD: Aunque usamos f-string para ORDER BY, validamos previamente
        que los valores estén en una lista permitida. Esto evita inyección SQL.
        Los valores de WHERE deben usar siempre placeholders %s.

        Args:
            ordenar_por (str): Columna para ordenar ('id', 'nombre', 'email', 'telefono')
        ...
        """
        if not self.connection: return []  # Guard clause: salir temprano si no hay conexión
        
        # -------------------------------------------------------------------------
        # VALIDACIÓN DE SEGURIDAD (prevención de inyección SQL)
        # NUNCA permitir que valores del usuario vayan directo a SQL sin validar
        # -------------------------------------------------------------------------
        columnas_permitidas = ['id', 'nombre', 'email', 'telefono']
        if ordenar_por not in columnas_permitidas:
            ordenar_por = 'nombre'  # Valor por defecto seguro
        
        direccion = direccion.upper()
        if direccion not in ['ASC', 'DESC']:
            direccion = 'ASC'  # Valor por defecto seguro
        
        try:
            cursor = self.connection.cursor()
            # NOTA: ORDER BY no admite placeholders %s en MySQL, por eso validamos arriba
            query = f"SELECT id, nombre, email, telefono FROM clientes ORDER BY {ordenar_por} {direccion}"
            cursor.execute(query)
            records = cursor.fetchall()  # fetchall() = traer todos los registros
            cursor.close()
            return records
        except Error as e:
